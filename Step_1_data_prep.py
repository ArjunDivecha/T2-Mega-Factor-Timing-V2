"""
# Factor Timing Data Preparation
# 
# INPUT FILES:
# - T2 Mega Factor Conditioning Variables.xlsx: Contains factor returns and conditioning variables
#   - Target sheet: 102 factor returns (49 CS and 53 TS variants) from Dec 1999 - Apr 2025
#   - Data from bloomberg sheet: US macroeconomic and market variables
#   - Factor 1MTR, 3MTR, 12MTR, 36MTR, 60MTR sheets: Trailing factor returns
#
# OUTPUT FILES:
...
#
# This script implements Phase 1 of the factor timing methodology:
...
INPUT_FILE = "T2 Mega Factor Conditioning Variables.xlsx"
...
def load_data(excel_file):
    ...
    # Load trailing returns at different horizons
    horizons = ["1MTR", "3MTR", "12MTR", "36MTR", "60MTR"]
    for horizon in horizons:
        sheet_name = f"Factor {horizon}"
        data[f'trailing_{horizon}'] = pd.read_excel(excel_file, sheet_name=sheet_name)
...
def handle_missing_values(data):
    ...
    # Handle missing values in trailing returns
    horizons = ["1MTR", "3MTR", "12MTR", "36MTR", "60MTR"]
    for horizon in horizons:
        key = f'trailing_{horizon}'
        df = data[key]
        df = df.apply(lambda x: x.fillna(x.mean()), axis=0)
        data[key] = df
        print(f"  {horizon} trailing returns missing after filling: {df.isna().sum().sum()}")
...
def create_factor_timing_portfolios(data):
    ...
    # Add trailing returns as predictors (1, 3, 12, 36, 60 months for each factor)
    horizons = ["1MTR", "3MTR", "12MTR", "36MTR", "60MTR"]
    trailing_predictors = []
    
    # Dictionary to store new trailing return predictors
    new_predictors_data = {}
    
    for horizon in horizons:
        trailing_returns = data[f'trailing_{horizon}']
        for factor in factors:
            if factor in trailing_returns.columns:
                new_col = f"{factor}_{horizon}"
                new_predictors_data[new_col] = stats.zscore(trailing_returns[factor].fillna(0))
                trailing_predictors.append(new_col)
    
    # Add all new trailing return predictors at once
    if new_predictors_data:
        trailing_df = pd.DataFrame(new_predictors_data, index=macro_vars_std.index)
        macro_vars_std = pd.concat([macro_vars_std, trailing_df], axis=1)
        predictors.extend(trailing_predictors)
...
def main():
    ...
    """
    print("=== FACTOR TIMING DATA PREPARATION ===")
    ...
    print("Ready to begin Phase 2 (Covariance Shrinkage Implementation)")
